...
35. 死锁、活锁、饥饿的区别？
36. 并发工具类Semaphore、`CountDownLatch`、`CyclicBarrier` 之间的区别？
37. 线程 B 怎么知道线程 A 修改了变量？ 线程之间的通信 （好多种方案）
    1.  volatile
    2. static
38. `volatile` 如何实现共享变量多线程的可见性？
39. 线程池的 7 大参数？核心线程可以被销毁么？
40. `submit`和`excute`有什么区别？
41. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`
42. 线程池的饱和策略有哪四种？
43. 新任务到线程池以后，它的运行流程怎么样？
44. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
45. `int++`是线程安全的么？
46. `ThreadLocal`的使用场景？
47. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
48. `ForkJoin` 有用过么？思想是什么？
